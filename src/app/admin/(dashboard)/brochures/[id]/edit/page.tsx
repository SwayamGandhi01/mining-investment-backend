...
export default function EditBrochurePage() {
  ...
  useEffect(() => {
    const fetchBrochure = async () => {
      try {
        const res = await axios.get(`/api/brochures/${id}`);
        if (res.data.success) {
  ...
  const onSubmit = async (data: BrochureInput) => {
    setSubmitting(true);
    try {
      const res = await axios.put(`/api/brochures/${id}`, data);
      if (res.data.success) {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-foreground">Edit Brochure</h1>
          <p className="text-sm text-muted mt-0.5">Update brochure details and PDF document</p>
        </div>
      </div>
