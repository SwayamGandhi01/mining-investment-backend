...
export default function AdminExhibitorsPage() {
  const [data, setData] = useState<ExhibitorItem[]>([]);
  ...
  const handleStatusToggle = async (id: string, newStatus: boolean | string) => {
    try {
      ...
      await axios.patch(`/api/exhibitors/${id}`, { status: statusStr });
      toast.success("Exhibitor status updated");
      fetchExhibitors();
    } catch {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await axios.delete(`/api/exhibitors/${deleteId}`);
  ...
  const columns: Column<ExhibitorItem>[] = [
    {
    ...
    },
    {
      ...
      cell: (item) => (
        <div className="flex items-center gap-1.5 text-xs text-muted">
          ...
          <span>{item.boothNumber || "Unassigned"}</span>
        </div>
      ),
    },
    {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted mt-0.5">Manage event booth exhibitors and showcase partners</p>
        </div>
        <Link
          href="/admin/exhibitors/create"
      ...
      <DataTable
        ...
        actions={(item) => (
          <div className="flex items-center justify-end gap-2">
            <Link
              href={`/admin/exhibitors/${item._id}/edit`}
      ...
      <ConfirmDialog
        ...
        title="Delete Multiple Exhibitors"
        message={`Are you sure you want to delete ${bulkIds.length} selected exhibitors?`}
        loading={deleting}
        onConfirm={async () => {
          setDeleting(true);
          try {
            await Promise.all(bulkIds.map((id) => axios.delete(`/api/exhibitors/${id}`)));
            toast.success("Exhibitors deleted");
            setBulkIds([]);
            fetchExhibitors();
          } catch {
            toast.error("Failed to delete exhibitors");
          } finally {
            setDeleting(false);
          }
        }}
