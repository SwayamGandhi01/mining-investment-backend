...
export async function GET(request: NextRequest) {
  try {
    ...
    const filter = buildFilterQuery(params, buildSearchQuery(params.search, ["title", "category", "description"]));
    const sort = buildSortQuery(params);
    const skip = getSkip(params.page, params.limit);

    const [data, total] = await Promise.all([
      Gallery.find(filter).sort(sort).skip(skip).limit(params.limit).populate("event", "title slug").lean(),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const slug = await ensureUniqueSlug(generateSlug(validation.data.title), "Gallery");
    ...
    return successResponse(newGallery, "Gallery album created successfully", 201);
  } catch (error) {
    return handleApiError(error);
  }
}
